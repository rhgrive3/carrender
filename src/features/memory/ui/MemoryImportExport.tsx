...
export function MemoryImportExport({ setId }: { setId?: string }) {
  ...
  const saveImport = async () => {
    if (!repository || rows.length === 0 || saving) return;
    setSaving(true);
    try {
      ...
      navigate(targetSetId ? { name: 'set', setId: targetSetId } : { name: 'home' });
    } catch (caught) {
      toast(caught instanceof Error ? caught.message : '取込に失敗しました');
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveSelectedSetImport = async () => {
    if (!repository || !selectedSetDocument || !selectedSetPreview || saving) return;
    ...
    try {
      const result = await importSelectedSetExport({
        repository,
        document: selectedSetDocument,
        includeStats: includeSelectedSetStats,
      });
      await refresh();
      void requestSync(true);
      ...
      toast(caught instanceof Error ? caught.message : '選択セットJSONを取り込めませんでした');
    } finally {
      setSaving(false);
    }
  };
  ...
  const exportAi = async () => {
    ...
    // AI-content documents are deliberately global. A set-scoped subset would
    // make every record outside that set appear as a deletion when the returned
    // JSON is compared with the full local knowledge base.
    const content = await repository.loadContent();
  ...
  const importAi = async () => {
    if (!repository || !aiPreview || saving) return;
    setSaving(true);
    try {
      const count = await applyAiImport({ repository, preview: aiPreview, selectedKeys: aiSelected, setId: targetSetId || undefined });
      await refresh();
      void requestSync(true);
      toast(`${count}件のAI差分を未確認データとして追加しました`);
      navigate(targetSetId ? { name: 'set', setId: targetSetId } : { name: 'home' });
    } catch (caught) {
      toast(caught instanceof Error ? caught.message : 'AI差分を追加できませんでした');
    } finally {
      setSaving(false);
    }
  };
    ...
    <section className="memory-import">
              ...
              <div className="memory-import-rows">
                ...
                {selectedSetPreview.statsAvailable > 0 && (
                  <label className="memory-ai-manual-note">
                    <input
                      type="checkbox"
                      checked={includeSelectedSetStats}
                      onChange={(event) => setIncludeSelectedSetStats(event.target.checked)}
                    />
          ...
          <div className="memory-sticky-actions">
            {selectedSetPreview ? (
              <button
                ...
                disabled={saving || parsing || parseErrors.length > 0 || selectedSetPreview.conflicts.some((conflict) => conflict.entityType !== 'stat' || includeSelectedSetStats)}
                onClick={() => void saveSelectedSetImport()}
              ><Upload size={18} />セットJSONを取り込む</button>
            ) : (
              <button type="button" className="btn btn-primary" disabled={saving || parsing || rows.length === 0 || parseErrors.length > 0} onClick={() => void saveImport()}><Upload size={18} />{rows.length}件を一括保存</button>
            )}
          </div>
        </div>
      )}
      ...
      {tab === 'export' && (
        <div className="memory-export-grid">
          <article className="card"><FileJson size={24} /><h3>ChatGPTへ手動で渡す</h3><p><b>AI API・APIキーは不要です。</b> 成績・履歴・ユーザー情報を除いたJSONファイルを取得し、ChatGPTアプリへ添付します。</p><ol className="memory-manual-ai-steps"><li>JSONを書き出す</li><li>依頼文をコピーしてChatGPTアプリへ貼る</li><li>返されたJSONを「AI差分」で確認して追加</li></ol><button type="button" className="btn btn-primary" onClick={() => void exportAi()}><Download size={18} />JSONファイルを取得</button><button type="button" className="btn btn-ghost" onClick={() => void copyChatGptRequest()}><Clipboard size={18} />依頼文をコピー</button><details className="memory-chatgpt-request"><summary>依頼文を表示</summary><pre>{CHATGPT_CONTENT_REQUEST}</pre></details></article>
          <article className="card"><CheckSquare size={24} /><h3>選択セット</h3><p>選択セットと参照コンテンツを出力します。統計は初期状態では含みません。</p><div className="field"><select aria-label="出力するセット" value={targetSetId} onChange={(event) => setTargetSetId(event.target.value)}><option value="">全セット</option>{sets.map((set) => <option key={set.id} value={set.id}>{set.name}</option>)}</select></div><label className="memory-ai-manual-note"><input type="checkbox" checked={includeSelectedExportStats} onChange={(event) => setIncludeSelectedExportStats(event.target.checked)} />統計も含める（回答履歴は含みません）</label><button type="button" className="btn btn-primary" onClick={() => void exportSelectedSet()}><Download size={18} />セットを書き出す</button></article>
          <article className="card"><ShieldAlert size={24} /><h3>完全バックアップ</h3><p>全コンテンツ・統計・回答履歴・セッションを復元専用形式で保存します。</p><button type="button" className="btn btn-primary" onClick={() => void exportBackup()}><Download size={18} />バックアップを作成</button></article>
          <MemoryBackupRestore />
        </div>
      )}
        ...
        <div className="memory-ai-import">
          ...
          {aiPreview && (
            <div className="card memory-ai-preview">
              ...
              <button type="button" className="btn btn-primary" disabled={saving || aiSelected.size === 0 || aiPreview.counts.invalid > 0} onClick={() => void importAi()}>選択項目だけ追加</button>
            </div>
          )}
        </div>
      )}
