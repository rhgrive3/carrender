...
interface PointerGesture { pointerId: number; x: number; y: number }
...
export function MemoryStudy({ sessionId }: { sessionId: string }) {
  ...
  const handlePointerUp = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    pointerGesture.current = null;
    if (!start || busy || !event.isPrimary || event.pointerId !== start.pointerId) return;
  ...
  return createPortal(
    <div ref={overlayRef} className="memory-study-overlay memory-simple-study" role="dialog" aria-modal="true" aria-label="暗記学習" tabIndex={-1}>
      ...
      <div className="memory-study-stage">
        <div className="memory-study-flip-shell" onPointerDown={(event) => { if (event.isPrimary && event.button === 0) pointerGesture.current = { pointerId: event.pointerId, x: event.clientX, y: event.clientY }; }} onPointerUp={handlePointerUp} onPointerCancel={() => { pointerGesture.current = null; }}>
