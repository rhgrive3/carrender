...
/** Clears one account by default. Omitting owner is reserved for tests/full reset. */
export function clearMainSyncMetadata(owner?: string): void {
  ...
  try {
    ...
    for (const key of keys) {
      if (key.startsWith(META_KEY_PREFIX) || key.startsWith(CONFLICT_BACKUP_KEY_PREFIX)) removeKey(key);
    }
  } catch {
    // Full cleanup is best effort and used only for tests or explicit reset.
  }
}
