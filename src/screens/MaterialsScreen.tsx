...
export function MaterialsScreen({ pane = 'materials', onPaneChange }: { pane?: MaterialsPane; onPaneChange?: (pane: MaterialsPane) => void }) {
  ...
  const startMaterialTimer = (material: Material) => {
    ...
    timer.start({ taskId: null, subjectId: material.subjectId, materialId: material.id, title: material.name, rangeLabel: material.name, sourceId: material.id });
  ...
  if (pane === 'memory') {
    return (
      <div className="screen memory-screen">
        <div className="screen-header memory-entry-header"><div><div className="screen-title">教材</div><div className="screen-sub">教材と暗記カードを管理</div></div></div>
  ...
  return (
    <div className="screen materials-screen">
      <div className="screen-header">
        <div><div className="screen-title">教材</div><div className="screen-sub">{archiveTab === 'active' ? `使用中 ${materials.length}冊` : `アーカイブ ${materials.length}冊`}</div></div>
