...
type RecordView = 'overview' | 'log';

function selectTabFromKeyboard<T extends string>(
...
export function RecordsScreen() {
  ...
  const [view, setView] = useState<RecordView>('log');
  ...
  return (
    <div className="screen records-v2">
      ...
      <div className="segmented record-view-switch" role="tablist" aria-label="記録画面の切替">
        <button id="record-tab-overview" role="tab" aria-controls="record-panel-overview" aria-selected={view === 'overview'} tabIndex={view === 'overview' ? 0 : -1} className={view === 'overview' ? 'active' : ''} onClick={() => setView('overview')} onKeyDown={(event) => selectTabFromKeyboard(event, ['overview', 'log'] as const, view, setView)}>集計</button>
        <button id="record-tab-log" role="tab" aria-controls="record-panel-log" aria-selected={view === 'log'} tabIndex={view === 'log' ? 0 : -1} className={view === 'log' ? 'active' : ''} onClick={() => setView('log')} onKeyDown={(event) => selectTabFromKeyboard(event, ['overview', 'log'] as const, view, setView)}>学習ログ</button>
      </div>

      {view === 'overview' && (
        <section id="record-panel-overview" role="tabpanel" aria-labelledby="record-tab-overview" tabIndex={0}>
          <div className="record-overview">
            <div className="segmented" role="tablist" aria-label="集計期間">
              <button id="record-period-tab-week" role="tab" aria-controls="record-period-panel" aria-selected={period === 'week'} tabIndex={period === 'week' ? 0 : -1} className={period === 'week' ? 'active' : ''} onClick={() => switchPeriod('week')} onKeyDown={(event) => selectTabFromKeyboard(event, ['week', 'month'] as const, period, switchPeriod)}>週</button>
              <button id="record-period-tab-month" role="tab" aria-controls="record-period-panel" aria-selected={period === 'month'} tabIndex={period === 'month' ? 0 : -1} className={period === 'month' ? 'active' : ''} onClick={() => switchPeriod('month')} onKeyDown={(event) => selectTabFromKeyboard(event, ['week', 'month'] as const, period, switchPeriod)}>月</button>
            </div>

            <div id="record-period-panel" role="tabpanel" aria-labelledby={period === 'week' ? 'record-period-tab-week' : 'record-period-tab-month'} tabIndex={0}>
              ...
              {period === 'week' ? (
                <WeekChart days={days} sessions={sessions} subjects={state.subjects} minutesByDay={minutesByDay} plannedByDay={plannedByDay} totalPlanned={totalPlanned} totalActual={totalActual} />
              ) : (
                ...
                <div className="card mt-12">
                  ...
                  {bySubject.map(({ subject, minutes }) => (
                    <div key={subject.id} className="row" style={{ marginBottom: 9 }}>
                      <span style={{ minWidth: 46, maxWidth: 88, fontSize: 12.5, fontWeight: 700, color: subject.color, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{subject.name}</span>
                      <div style={{ flex: 1, height: 9, background: 'var(--bg-elev3)', borderRadius: 100 }}>
                        <div style={{ width: `${(minutes / maxSubject) * 100}%`, height: '100%', borderRadius: 100, background: subject.color, transition: 'width 0.5s ease' }} />
                      </div>
      ...
      {view === 'log' && (
        <section id="record-panel-log" role="tabpanel" aria-labelledby="record-tab-log" tabIndex={0} className="record-log-view">
...
function WeekChart({ days, sessions, subjects, minutesByDay, plannedByDay, totalPlanned, totalActual }: { days: string[]; sessions: StudySession[]; subjects: Subject[]; minutesByDay: Map<string, number>; plannedByDay: Map<string, number>; totalPlanned: number; totalActual: number }) {
  const t = today();
    ...
    <div className="card mt-12 studyplus-chart-card">
      ...
      <div className="studyplus-chart">
        {days.map((d) => {
          ...
          const future = d > t;
          const stacks = [...(subjectMinutes.get(d)?.entries() ?? [])].sort((a, b) => b[1] - a[1]).map(([subjectId, minutes]) => ({ subject: resolveRecordSubject(subjects, subjectId), minutes }));
          return (
            <div key={d} className={`studyplus-day ${future ? 'future' : ''}`}>
              ...
              <div className="studyplus-bar-area">
                <div className="studyplus-plan-rail" title={`予定 ${formatMinutes(planned)}`} style={{ height: `${Math.min(100, (planned / maxDay) * 100)}%` }} />
                <div className="studyplus-actual-bar" title={`実績 ${formatMinutes(actual)}`} style={{ height: `${Math.min(100, (actual / maxDay) * 100)}%` }}>
                  {stacks.map(({ subject, minutes }) => (
                    <div key={subject.id} className="studyplus-stack" title={`${subject.name} ${formatMinutes(minutes)}`} style={{ height: `${recordChartSharePercent(minutes, actual)}%`, background: subject.color }} />
                  ))}
                </div>
              </div>
              <span className={`studyplus-weekday ${d === t ? 'today' : ''}`}>{WEEKDAY_LABELS[weekdayOf(d)]}</span>
            </div>
          );
        })}
      </div>
      <div className="studyplus-legend">
        <span className="studyplus-legend-item muted-legend"><span className="studyplus-legend-rail" />予定</span>
        {visibleSubjects.map(({ subject }) => (
          <span key={subject.id} className="studyplus-legend-item"><span className="studyplus-legend-dot" style={{ background: subject.color }} />{subject.name}</span>
        ))}
      </div>
    </div>
  );
...
function MonthHeatCalendar({ month, minutesByDay }: { month: string; minutesByDay: Map<string, number> }) {
  ...
  return (
    <div className="card mt-12" style={{ padding: 13 }}>
      <MonthCalendar month={month} renderDay={(d) => { const min = minutesByDay.get(d) ?? 0; return <span className="cal-cell-min">{formatMinutesCompact(min)}</span>; }} cellStyle={(d) => { const min = minutesByDay.get(d) ?? 0; if (min <= 0) return undefined; const ratio = Math.min(1, min / max); return { background: `color-mix(in srgb, var(--bg-elev1), var(--accent) ${Math.round(12 + ratio * 55)}%)` }; }} />
    ...
    if (s.date !== lastDate) {
      ...
      out.push(
        <div key={`h-${s.date}`} className="row spread" style={{ margin: '10px 2px 6px' }}>
          <span style={{ fontSize: 12.5, fontWeight: 800 }}>{s.date === t ? '今日' : `${formatDateShort(s.date)} (${WEEKDAY_LABELS[weekdayOf(s.date)]})`}</span>
      ...
      <button type="button" className="task-card session-log-button" key={s.id} onClick={() => onEdit(s)} aria-label={`${material?.name ?? s.rangeLabel ?? '学習'}の記録を編集`}>
        ...
        <div className="task-main">
          <div className="task-meta-row">
            <span className="subject-chip" style={{ background: `color-mix(in srgb, ${subject.color}, transparent 85%)`, color: subject.color }}>{subject.name}</span>
            <span className="task-type-chip iflex" style={{ gap: 3 }}>
              {s.source === 'timer' ? <><Timer size={11} strokeWidth={2.4} aria-hidden="true" /> タイマー</> : <><PenLine size={11} strokeWidth={2.4} aria-hidden="true" /> 手入力</>}
            </span>
          </div>
