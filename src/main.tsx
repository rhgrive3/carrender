...
function RecordTabPanelSemanticsGuard() {
  React.useEffect(() => {
    let scheduledFrame = 0;

    const tabByLabel = (group: Element | null, label: string) => {
      if (!group) return undefined;
      return [...group.querySelectorAll<HTMLElement>('[role="tab"]')]
        .find((tab) => tab.textContent?.trim() === label);
    };

    const connectRecordTabsToPanels = () => {
      scheduledFrame = 0;
    ...
    const scheduleConnection = () => {
      if (scheduledFrame) return;
      scheduledFrame = requestAnimationFrame(connectRecordTabsToPanels);
    };
