...
assert.match(appContext, /function targetMatchesTask[\s\S]*target\.sourceId !== task\.sourceId[\s\S]*target\.range\.start === range\.start/, '再計算でIDが変わっても作業系列と範囲から同じ計測対象を照合する');
assert.match(appContext, /function locatorMatchesTarget[\s\S]*locator\.sourceId[\s\S]*target\.materialId !== materialId/, '記録側のlocatorも教材と作業系列を含めて照合する');
assert.match(appContext, /action\.type === 'RECORD_SESSION'[\s\S]*action\.input\.source !== 'timer'[\s\S]*targetMatchesSessionInput/, '計測中タスクの手動完了記録を中央境界で拒否する');
assert.match(appContext, /action\.type === 'UPDATE_SESSION'[\s\S]*targetMatchesSession\(target, previous\)/, '編集後に参照を外しても編集前の計測中タスクを保護する');
assert.match(appContext, /action\.type === 'DELETE_SESSION'[\s\S]*targetMatchesSession\(target, previous\)/, '計測中タスクに紐づく既存記録の削除を拒否する');
