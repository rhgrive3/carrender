...
assert.match(source, /if \(actionItemId\) \{[\s\S]*saveRoutedMemoryItemDraft\([\s\S]*expectedItemId: actionItemId[\s\S]*\} else \{[\s\S]*saveNewMemoryItemCards\(/u, '既存Item編集はroute Item identityへ固定し、新規追加は各カードを独立Item保存へ分ける');
assert.match(source, /savedCount > 1 \? `\$\{savedCount\}枚のカードを保存しました`/u, '複数カード保存時は保存枚数を利用者へ伝える');
assert.match(source, /\{itemId \? '別の意味を追加' : '別のカードを追加'\}/u, '既存ItemではSense追加、新規画面ではカード追加と正しく案内する');
assert.match(source, /if \(!isCurrentAction\(\)\) return;[\s\S]*await refresh\(\);[\s\S]*if \(!isCurrentAction\(\)\) return;[\s\S]*requestSync\(true\)/u, '所有者切替後に旧保存から現在一覧の更新や同期を実行しない');
...
assert.match(source, /const nextBlank = blankDraft\(\);[\s\S]*setOriginal\(undefined\);[\s\S]*setDraft\(nextBlank\);[\s\S]*setLoadError\(undefined\)/u, '編集対象切替時に旧カード内容を消して空の保存基準へ戻す');
