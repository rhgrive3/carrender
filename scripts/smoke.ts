...
// このスモークは実時刻で走る: 夜遅くは今日の残り空き時間が尽きているため、
// 今日に60分以上の空きがなければ翌日を基準に「直近の計画日が埋まっている」ことを確認する
{
  const remainingToday = futureFreeSlotsOn(state, t, new Date()).reduce((sum, s) => sum + s.end - s.start, 0);
  const refDay = remainingToday >= 60 ? t : addDays(t, 1);
  const refTasks = state.tasks.filter((x) => x.scheduledDate === refDay && x.status === 'planned' && x.scheduledStart);
  check(`直近の計画日(${refDay})にタスクが生成されている`, refTasks.length >= 2, { remainingToday, refTasks: refTasks.length });
}
...
console.log('--- 固定予定・完了タスクとの時間帯重複防止 ---');
{
  ...
  // 完了済みタスクは履歴であり、未来日の空き時間・予算を予約しない。
  // 計画対象(planned)のタスク同士だけが重複禁止で、完了済みタスクは保持される。
  const plannedDayTasks = dayTasks.filter((x) => x.status === 'planned');
  const overlapping = plannedDayTasks.filter((a) =>
    plannedDayTasks.some((b) => {
  ...
  check(
    ...
    plannedDayTasks.map((x) => `${x.status} ${x.scheduledStart}〜${x.scheduledEnd}`),
  );
  check('完了済み未来タスクは保持される', replanned.tasks.some((x) => x.id === 'task_done_ov' && x.status === 'done'));
  check('完了済み未来タスクを固定衝突にしない', (replanned.lastScheduleResult?.conflicts.length ?? 0) === 0, replanned.lastScheduleResult?.conflicts);
  // 完了済みタスク(18:00〜18:50)の時間帯が未来の空き時間として解放されている
  // (=固定予約として容量から差し引かれない)ことを、その帯への新規配置で確認する
  const doneSlotFreed = plannedDayTasks.some((x) => {
    const r = toRange(x);
    return r.start < 18 * 60 + 50 && r.end > 18 * 60;
  });
  check('完了済み未来タスクが空き時間を予約しない(同時間帯へ新規配置できる)', doneSlotFreed, plannedDayTasks.map((x) => `${x.scheduledStart}〜${x.scheduledEnd}`));
...
console.log('--- 計画ロジックの品質(上書き・ピン留め・ローテーション・実績反映) ---');
{
  ...
  const pinned: StudyTask = {
    ...
    createdAt: fixedNow.toISOString(),
    completedAt: null,
    // マイグレーション(normalizeState)が旧手動タスクへ付与する明示ロック。
    // スケジューラーは実行時にgeneratedByからロックを推測しない。
    placementLock: 'time',
  };
...
console.log('--- V2 制約・決定性 ---');
{
  ...
  // 配置できない教材は全ロールバックされるので、不足は試行中の仮配置を引かず全量(360分)で報告する
  check('V2: 配置不能な厳守教材の不足を全量360分と報告する', shortage.objectiveReport.unscheduledStrictMinutes === 360, shortage.objectiveReport);
  const shortageReports = shortage.deadlineReports.filter((r) => r.policy === 'strict');
  check('V2: 実行可能な方の厳守教材は期限内に全量配置される', shortageReports.some((r) => r.feasible === true && r.shortageMinutes === 0), shortageReports);
  check('V2: 配置不能な厳守教材はscheduledMinutes=0', shortageReports.some((r) => r.feasible === false && r.scheduledMinutes === 0 && r.shortageMinutes === 360), shortageReports);
