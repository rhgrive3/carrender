...
const recoveryRevokeAt = recoverySource.indexOf('URL.revokeObjectURL(url);', recoveryDelayAt);
...
assert.match(recoverySource, /document\.body\.appendChild\(link\);[\s\S]*link\.click\(\);[\s\S]*window\.setTimeout\(\(\) => \{[\s\S]*link\.remove\(\);[\s\S]*URL\.revokeObjectURL\(url\);/u, 'iOS向けにanchorをDOMへ追加し、遅延cleanupする');
