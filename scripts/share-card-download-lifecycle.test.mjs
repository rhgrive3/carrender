...
const buildAt = source.indexOf('const blob = await buildShareCard(state, ref);', shareFunctionAt);
...
assert.match(source, /function canvasToPngBlob\(canvas: HTMLCanvasElement\): Promise<Blob \| null>[\s\S]*canvas\.toBlob\(resolve, 'image\/png'\)/u, 'Canvasを非同期toBlobでPNGへ変換する');
assert.match(source, /async function buildShareCard[\s\S]*return canvasToPngBlob\(canvas\);/u, '描画後のPNG圧縮を非同期Promiseとして返す');
assert.equal(source.includes("canvas.toDataURL('image/png')"), false, '同期toDataURLを使わない');
assert.equal(source.includes('atob('), false, 'base64を全件decodeしない');
assert.equal(source.includes('new Uint8Array(bin.length)'), false, 'PNG全byteの再コピーを作らない');
...
console.log('✅ async share card and recovery download lifecycle regressions passed');
