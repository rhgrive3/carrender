...
assert.match(home, /disabled=\{syncStatus === 'syncing'\}[\s\S]*aria-busy=\{syncStatus === 'syncing'\}[\s\S]*aria-label=\{syncStatus === 'syncing' \? '暗記データを同期中' : '暗記データを同期'\}/u, '手動同期中は再操作を止めてボタン自体へ処理中状態を通知する');
