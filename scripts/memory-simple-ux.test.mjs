...
assert.match(study, /memorySwipeDirection/u, '左右スワイプを縦移動量とpointer識別を含めて判定する');
...
assert.match(studyPolish, /:has\(\.memory-study-card\[data-card-side='answer'\]\) \.memory-study-stage \{[\s\S]*overflow-y:\s*auto[\s\S]*-webkit-overflow-scrolling:\s*touch/u, '答え面は長い例文を最後まで縦スクロールできる');
assert.match(studyPolish, /:has\(\.memory-study-card\[data-card-side='answer'\]\) \.memory-study-flip-shell \{[\s\S]*touch-action:\s*pan-y/u, '答え面の縦スクロールをブラウザへ許可する');
assert.match(studyPolish, /\.memory-again \{\s*--assessment-tone:\s*#a95f5f/u, 'まだは彩度を抑えた赤にする');
