...
assert.match(setup, /const \{ repository, ready, sets, navigate, refresh, requestSync \} = useMemory\(\)/u, '暗記データ準備完了後にセット選択を検証し、開始後の同期手段も取得する');
...
assert.match(studyPolish, /:has\(\.memory-study-card\[data-card-side='question'\]\) \.memory-study-card-front \{[\s\S]*overflow:\s*hidden/u, '問題カード自身にもスクロール領域を残さない');
assert.match(studyPolish, /:is\(\.memory-again, \.memory-partial\)[\s\S]*color-mix[\s\S]*var\(--bg-elev1\)/u, 'まだ・あやしいは強い全面色ではなく落ち着いたトーン面にする');
assert.match(studyPolish, /\.memory-again \{\s*--assessment-tone:\s*#bd6078/u, 'まだは彩度を抑えたローズ色にする');
