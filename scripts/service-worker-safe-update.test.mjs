...
assert.match(policy, /const disabled = updateApplying \|\| blocked/u, '操作途中と適用中はskipWaiting操作を無効化する');
assert.match(policy, /async function applyWaitingUpdate[\s\S]*serviceWorkerUpdateBlockers\(\)[\s\S]*renderNotice\(\)[\s\S]*return/u, 'click直前にもblockerを再検証する');
assert.match(policy, /updateServiceWorker\(true\)/u, '安全な利用者操作だけでwaiting workerを適用しreloadする');

assert.match(policy, /dataset\.renderSignature === signature[\s\S]*return/u, '同一notice状態ではDOMを再生成しない');
assert.match(policy, /mutationTouchesOnlyNotice[\s\S]*mutations\.every[\s\S]*return/u, 'notice自身のmutationでobserverを自己発火させない');
assert.match(policy, /await updateServiceWorker\(true\)[\s\S]*catch \(caught\)[\s\S]*updateError[\s\S]*renderNotice\(\)/u, '更新失敗を処理し再試行可能な表示へ戻す');
assert.match(policy, /buttonLabel = updateApplying[\s\S]*更新を再試行/u, '適用失敗後に再試行操作を表示する');
