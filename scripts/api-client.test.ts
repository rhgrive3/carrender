...
globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (init?.method === 'GET') {
    return jsonResponse({ format: 'chunked-v1', generationId, updatedAt: '2026-07-14T00:00:00.000Z', manifest: encoded.manifest });
  }
...
globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (body.action === 'begin') {
    return jsonResponse({
      generationId: 'generation-upload-test',
      status: 'staging',
      updatedAt: null,
  ...
  if (body.action === 'commit') {
    return jsonResponse({ ok: true, generationId: 'generation-upload-test', updatedAt: '2026-07-14T00:00:01.000Z' });
  }
...
try {
  ...
  generic503Error = caught as ApiError;
}
