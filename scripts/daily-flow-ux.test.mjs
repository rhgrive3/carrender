...
assert.match(taskRowSource, /const isDoing = task\.status === 'doing'/, '通常タスク行でも計測中状態を識別する');
assert.match(taskRowSource, /!isDoing && \([\s\S]*aria-label=\{`\$\{task\.title\}を延期`\}/, '計測中タスクの延期操作を通常行にも表示しない');
assert.match(taskRowSource, /if \(isDoing\) \{[\s\S]*計測中のタスクは延期できません/, '操作関数を直接呼ばれても計測中タスクを延期しない');
assert.match(taskRowSource, /isDoing \? '続ける' : '開始'/, '計測中タスクは開始ではなく続行として案内する');
