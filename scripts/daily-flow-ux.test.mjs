...
assert.match(recordSource, /const recentTargets = useMemo\(\(\) => \{/, '最近の教材候補は内部計算として維持する');
assert.match(recordSource, /最近使った教材/, '既存の候補UI構造を検出できる');
...
assert.match(recordFixSource, /手入力記録は入力欄へ直行させる/, '手入力では最近の教材候補を画面に出さない');
assert.match(recordFixSource, /\.sheet > \.field:has\(> label:first-child \+ \.row > \.btn\.btn-secondary\.btn-sm \.faint\)/, '最近使った教材の候補フィールドだけを非表示にする');
