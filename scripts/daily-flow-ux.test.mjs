...
assert.match(uxSource, /手入力記録は入力欄へ直行させる/, '手入力では最近の教材候補を画面に出さない');
assert.match(uxSource, /\.sheet > \.field:has\(> label:first-child \+ \.row > \.btn\.btn-secondary\.btn-sm \.faint\)/, '最近使った教材の候補フィールドだけを非表示にする');
