...
assert.match(
  recordSheet,
  /\{\(material \|\| \(activeTaskTarget && task\) \|\| session\?\.materialId\) && \([\s\S]*ariaLabel="今回やった量"/,
  '削除済み教材を参照する過去記録でも今回やった量を編集できる',
);
assert.match(
  recordSheet,
  /session && hasTaskTarget && !keepsSameTaskTarget[\s\S]*元のタスクとの紐付けを解除します/,
  '参照先を変更した時だけ旧タスクの完了UIを外して影響を説明する',
);
