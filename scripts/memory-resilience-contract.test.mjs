...
assert.match(studySource, /const beginAction = \(\) => \{[\s\S]*?if \(actionInFlight\.current\) return false;[\s\S]*?actionInFlight\.current = true/, '回答・取り消しの多重実行を同期的に拒否する');
assert.match(studySource, /const finishAction = \(\) => \{[\s\S]*?actionInFlight\.current = false;[\s\S]*?setBusy\(false\)/, '処理完了時にsingle-flightロックと表示状態を解除する');
assert.equal((studySource.match(/!beginAction\(\)/g) ?? []).length, 2, '回答保存と取り消しの両方がsingle-flightロックを使う');
assert.equal((studySource.match(/const actionSessionId = session\.id;/g) ?? []).length, 2, '回答保存と取り消しが開始時のセッションIDを固定する');
assert.match(studySource, /if \(!mounted\.current \|\| activeSessionId\.current !== actionSessionId\) return;[\s\S]*?setSession\(result\.session\)/, '古い回答保存完了で切替後のセッション状態を上書きしない');
assert.match(studySource, /if \(!mounted\.current \|\| activeSessionId\.current !== actionSessionId\) return;[\s\S]*?setSession\(restored\.session\)/, '古い取り消し完了で切替後のセッション状態を上書きしない');
...
assert.match(studySource, /mounted\.current && activeSessionId\.current === actionSessionId/, '別セッションへ切替後に古い操作のToastを出さない');
