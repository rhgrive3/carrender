...
import { draftFromContent } from '../src/features/memory/ui/MemoryEditor';
...
const exerciseContent: MemoryContentBundle = {
  ...content,
  exercises: [
    {
      id: 'exercise-later', senseId: 'sense-1', answerId: 'answer-1', type: 'translation', prompt: '好奇心を引き出すを英訳せよ',
      context: '授業について', acceptedAnswerIds: ['answer-1'], requiredTokens: ['tap', 'into'], forbiddenTokens: ['draw'],
      explanation: 'tap intoを使う', hint: 'tapから始める', siblingGroupId: 'sibling-1', source: 'user', verificationStatus: 'verified',
      createdAt: '2026-07-21T00:00:02.000Z', updatedAt: now, revision: 2,
    },
    {
      id: 'exercise-first', senseId: 'sense-1', answerId: 'answer-1', type: 'translation', prompt: '創造性を引き出すを英訳せよ',
      acceptedAnswerIds: ['answer-1'], requiredTokens: [], forbiddenTokens: [], siblingGroupId: 'sibling-1', source: 'user', verificationStatus: 'verified',
      createdAt: '2026-07-21T00:00:01.000Z', updatedAt: now, revision: 1,
    },
  ],
};
const exerciseDraft = draftFromContent(exerciseContent, 'item-1');
assert.ok(exerciseDraft, '既存Itemから編集draftを作れる');
assert.deepEqual(exerciseDraft.senses[0].exercises?.map((exercise) => exercise.id), ['exercise-first', 'exercise-later'], '非表示ExerciseをcreatedAtとIDの安定順序で保持する');
assert.equal(exerciseDraft.senses[0].exercises?.[1].answerIndex, 0, 'ExerciseのanswerIdを編集draftのAnswer indexへ復元する');
assert.deepEqual(exerciseDraft.senses[0].exercises?.[1].acceptedAnswerIndexes, [0], 'acceptedAnswerIdsも同じAnswer indexへ復元する');
assert.equal(exerciseDraft.senses[0].exercises?.[1].requiredTokens, 'tap, into', 'required tokenを保存形式へ往復できる文字列へ変換する');
...
await saveMemoryItemDraft({
  ...
  draft: {
    ...exerciseDraft,
    senses: exerciseDraft.senses.map((sense, index) => index === 0 ? { ...sense, promptJa: '能力を引き出す', meaningJa: '能力を引き出す' } : sense),
  },
});
const savedExercises = saved.filter((entry) => entry.entityType === 'exercise');
assert.equal(savedExercises.length, 2, '日本語だけの編集でも既存Exerciseを両方保存対象へ維持する');
