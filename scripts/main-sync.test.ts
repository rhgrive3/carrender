...
check('owner指定の消去で対象アカウントだけ削除する',
...
clearMainSyncMetadata();
check('owner未指定のログアウト互換消去はowner scoped情報を保持する',
  getMainSyncMetadata(owner)?.dirty === false && getMainSyncMetadata(otherOwner)?.dirty === true);
