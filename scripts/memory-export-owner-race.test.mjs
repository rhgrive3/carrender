...
assert.match(source, /await actionRepository\.loadContent\(\);[\s\S]*if \(!isCurrentRepository\(actionRepository\)\) return;[\s\S]*downloadJson/u, 'AI用JSONは所有者切替後にダウンロードしない');
assert.match(source, /await actionRepository\.loadSnapshot\(\);[\s\S]*if \(!isCurrentRepository\(actionRepository\)\) return;[\s\S]*createSelectedSetExport/u, '選択セットは所有者切替後にダウンロードしない');
assert.match(source, /await actionRepository\.exportAll\(\);[\s\S]*if \(!isCurrentRepository\(actionRepository\)\) return;[\s\S]*createFullMemoryBackup/u, '完全バックアップは所有者切替後にダウンロードしない');
...
assert.doesNotMatch(source, /const content = await repository\.loadContent\(\);[\s\S]*downloadJson/u, '無保護の旧出力構造へ戻さない');
