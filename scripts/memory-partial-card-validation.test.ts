...
const blankSense = (): MemoryItemDraft['senses'][number] => ({
  promptJa: '',
  answers: [{ displayForm: '' }],
  examples: [],
});
...
const validCard = (update: Partial<MemoryItemDraft['senses'][number]> = {}): MemoryItemDraft['senses'][number] => ({
  promptJa: '唖然とした',
  answers: [{ displayForm: 'appalled' }],
  examples: [],
  ...update,
});

assert.throws(
  () => selectValidMemorySenseDrafts([validCard({
    answers: [{ displayForm: '', citationForm: 'appall', note: '補足' }],
  })], 'カード'),
  /カード1の英語1を入力してください/,
  'metadataだけ残った新規Answerを黙って捨てない',
);
assert.throws(
  () => selectValidMemorySenseDrafts([validCard({
    answers: [{ id: 'answer-existing', displayForm: '' }],
  })], '意味'),
  /意味1の英語1が空です。削除する場合は削除ボタンを使ってください/,
  '既存Answerの必須field空欄化を暗黙削除へ変換しない',
);
assert.throws(
  () => selectValidMemorySenseDrafts([validCard({
    examples: [{ english: '', japanese: '彼は唖然とした。' }],
  })], 'カード'),
  /カード1の例文1の英語を入力してください/,
  '和訳だけの新規例文を黙って捨てない',
);
assert.throws(
  () => selectValidMemorySenseDrafts([validCard({
    examples: [{ id: 'example-existing', english: '' }],
  })], '意味'),
  /意味1の例文1の英語が空です。削除する場合は削除ボタンを使ってください/,
  '既存例文の英語空欄化を暗黙削除へ変換しない',
);
assert.throws(
  () => selectValidMemorySenseDrafts([validCard({
    exercises: [{ type: 'translation', prompt: '', hint: 'appalledを使う' }],
  })], 'カード'),
  /カード1の問題1の問題文を入力してください/,
  'hintだけの新規Exerciseを黙って捨てない',
);
assert.throws(
  () => selectValidMemorySenseDrafts([validCard({
    exercises: [{ id: 'exercise-existing', type: 'translation', prompt: '' }],
  })], '意味'),
  /意味1の問題1の問題文が空です。削除する場合は削除ボタンを使ってください/,
  '既存Exerciseの問題文空欄化を暗黙削除へ変換しない',
);
assert.equal(selectValidMemorySenseDrafts([validCard({
  answers: [{ displayForm: 'appalled' }, { displayForm: '' }],
  examples: [{ english: '' }],
  exercises: [{ type: 'translation', prompt: '' }],
})], 'カード').length, 1, '完全空の新規nested rowは従来どおり無視できる');
...
await assert.rejects(
  saveNewMemoryItemCards({
    ...
    draft: {
      kind: 'word',
      senses: [validCard({ examples: [{ english: '', japanese: '彼は唖然とした。' }] })],
    },
  }),
  /カード1の例文1の英語を入力してください/,
);
assert.equal(saveCalls, 0, 'nested rowの検証失敗でも新規保存transactionを開始しない');
...
const original: MemoryContentBundle = {
  ...
  examples: [{
    id: 'example-1', senseId: 'sense-1', answerId: 'answer-1', english: 'First example.', japanese: '最初の例文。',
    source: 'user', verificationStatus: 'verified', createdAt: now, updatedAt: now, revision: 1,
  }],
  exercises: [{
    id: 'exercise-1', senseId: 'sense-1', answerId: 'answer-1', type: 'translation', prompt: '第一を英訳せよ',
    acceptedAnswerIds: ['answer-1'], requiredTokens: [], forbiddenTokens: [], siblingGroupId: 'siblings',
    source: 'user', verificationStatus: 'verified', createdAt: now, updatedAt: now, revision: 1,
  }],
};
...
await assert.rejects(
  saveMemoryItemDraft({
    ...
    draft: {
      id: 'item-1', kind: 'word', label: 'first', lemma: 'first', senses: [{
        id: 'sense-1', promptJa: '第一',
        answers: [{ id: 'answer-1', displayForm: 'first' }],
        examples: [{ id: 'example-1', english: '', japanese: '最初の例文。' }],
      }],
    },
  }),
  /意味1の例文1の英語が空です。削除する場合は削除ボタンを使ってください/,
);
assert.equal(saveCalls, 0, '既存nested rowの空欄化でもtransactionを開始しない');
...
assert.equal(savedEntities.some((entry) => entry.entityType === 'example'
  && entry.entityId === 'example-1'
  && entry.operation === 'delete'), true, '明示削除されたExampleのtombstone契約を維持する');
assert.equal(savedEntities.some((entry) => entry.entityType === 'exercise'
  && entry.entityId === 'exercise-1'
  && entry.operation === 'delete'), true, '明示削除されたExerciseのtombstone契約を維持する');
...
console.log('✅ partial memory cards and nested rows are rejected before save while truly blank rows remain ignorable');
