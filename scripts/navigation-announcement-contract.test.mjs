...
assert.match(source, /function textFromIdRefs\(value: string \| null\)/, 'aria-labelledby references must be resolved through a dedicated helper');
assert.match(source, /split\(\/\\s\+\/\)/, 'multiple whitespace-separated aria-labelledby IDs must be supported');
assert.match(source, /document\.getElementById\(id\)\?\.textContent\?\.trim\(\)/, 'every referenced label node must contribute its text');
...
assert.match(source, /new MutationObserver\(scheduleAnnouncement\)/, 'screen changes must be batched instead of rescanning for every mutation');
assert.match(source, /if \(scheduledFrame\) return;/, 'only one navigation scan may be queued per animation frame');
assert.match(source, /scheduledFrame = requestAnimationFrame\(announceCurrentScreen\)/, 'batched screen naming must run on the next paint boundary');
assert.match(source, /characterData: true/, 'text-only title changes must update the current screen name');
...
assert.match(source, /if \(scheduledFrame\) cancelAnimationFrame\(scheduledFrame\)/, 'a queued scan must not run after unmount');
