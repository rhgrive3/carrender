...
const stateSnapshot = readFileSync(new URL('../src/lib/appStateSnapshot.ts', import.meta.url), 'utf8');
...
assert.match(chartGuard, /getAppStateSnapshot\(\)[\s\S]*subscribeAppStateSnapshot\(schedule\)/, 'Reactでcommit済みの現在stateを購読して代替表を更新する');
assert.doesNotMatch(chartGuard, /loadState|localStorage|IndexedDB/, 'グラフ代替表が永続化snapshotを画面データ源として再読込しない');
assert.match(chartGuard, /if \(!state\) \{[\s\S]*clearChartAlternatives\(\)/, 'owner切替・unmount時に前stateの代替表を残さない');
assert.match(chartGuard, /requestAnimationFrame[\s\S]*subscribeAppStateSnapshot[\s\S]*MutationObserver/, 'state更新とlazy描画を同じframeへ集約する');
assert.match(stateSnapshot, /publishAppStateSnapshot[\s\S]*current = \{ owner, state \}[\s\S]*notify\(\)/, '最新stateとownerを同じgenerationとして公開する');
assert.match(stateSnapshot, /clearAppStateSnapshot\(owner\?[\s\S]*current\.owner !== owner[\s\S]*current = null/, '別ownerのcleanupで現在snapshotを誤消去しない');
assert.match(appContext, /useLayoutEffect\(\(\) => \{\s*publishAppStateSnapshot\(owner, base\.state\);\s*\}, \[base\.state, owner\]\)/, 'React commit時に最新AppStateを同期公開する');
assert.match(appContext, /useLayoutEffect\(\(\) => \(\) => \{\s*clearAppStateSnapshot\(owner\);\s*\}, \[owner\]\)/, 'owner切替・unmount時に前ownerのsnapshotを破棄する');
