...
assert.match(source, /<section className="memory-import" aria-busy=\{saving\}>/u, '取込画面全体の処理中状態を支援技術へ伝える');
assert.match(source, /<fieldset disabled=\{saving\}/u, '保存中は取込内容と保存先を変更できない');
assert.match(source, /aria-label="戻る" disabled=\{saving\}/u, '保存中の画面離脱を防ぐ');
