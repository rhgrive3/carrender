...
const safeSyncAfterInvalidation = String.raw`(?:requestSyncSafely\(false\)|void requestSync\(false\)\.catch\(\(\) => undefined\))`;
assert.match(source, new RegExp(String.raw`saveSession\([\s\S]*?status: 'abandoned'[\s\S]*?try \{\s*await refresh\(\);\s*\} catch \(caught\) \{[\s\S]*?${safeSyncAfterInvalidation}[\s\S]*?旧形式の問題セッションは廃止されました`, 'u'), '旧形式セッションでは一覧更新失敗後も終了状態を安全に同期して本来の案内を維持する');
assert.match(source, new RegExp(String.raw`sessionContentIsRestorable[\s\S]*?saveSession\([\s\S]*?status: 'abandoned'[\s\S]*?try \{\s*await refresh\(\);\s*\} catch \(caught\) \{[\s\S]*?${safeSyncAfterInvalidation}[\s\S]*?学習中のカードが編集`, 'u'), '復元不能セッションでは一覧更新失敗後も終了状態を安全に同期して本来の案内を維持する');
assert.equal(source.includes("await refresh();\n        throw new Error('旧形式の問題セッション"), false, '一覧更新失敗を旧形式セッション終了失敗として扱わない');
assert.equal(source.includes("await refresh();\n        throw new Error('学習中のカードが編集"), false, '一覧更新失敗を復元不能セッション終了失敗として扱わない');
...
assert.match(source, /useLayoutEffect\(\(\) => \{\s*actionToken\.current \+= 1;[\s\S]*?actionInFlight\.current = false;[\s\S]*?pointerStart\.current = null;[\s\S]*?ignoreNextClick\.current = false;[\s\S]*?setSession\(undefined\);[\s\S]*?setBundle\(undefined\);[\s\S]*?setLoadError\(undefined\);[\s\S]*?setRevealed\(false\);[\s\S]*?setFlipDirection\(undefined\);[\s\S]*?setBusy\(false\);\s*\}, \[repository, sessionId\]\)/u, '所有者またはセッション切替時に旧カード・操作・二軸ポインター状態を描画前に破棄する');
