...
assert.match(source, /saveSession\([\s\S]*?status: 'abandoned'[\s\S]*?try \{\s*await refresh\(\);\s*\} catch \(caught\) \{[\s\S]*?requestSyncSafely\(false\)[\s\S]*?旧形式の問題セッションは廃止されました/u, '旧形式セッションでは一覧更新失敗後も終了状態を同期して本来の案内を維持する');
assert.match(source, /sessionContentIsRestorable[\s\S]*?saveSession\([\s\S]*?status: 'abandoned'[\s\S]*?try \{\s*await refresh\(\);\s*\} catch \(caught\) \{[\s\S]*?requestSyncSafely\(false\)[\s\S]*?学習中のカードが編集または削除されました/u, '復元不能セッションでは一覧更新失敗後も終了状態を同期して本来の案内を維持する');
