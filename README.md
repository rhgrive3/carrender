...
**モバイルのブラウザで開くと全画面のインストールゲートが表示され、ホーム画面に追加するまでアプリは使えません。**

- **iPhone / iPad**: Safariで開く → 共有ボタン → 「ホーム画面に追加」(ゲートに手順を表示)
- **Android**: 「ホーム画面に追加する」ボタン1タップでインストール(`beforeinstallprompt`)
- **LINE等のアプリ内ブラウザ**: 追加不可のため、Safari/Chromeで開き直す手順とURLコピーを案内
- **デスクトップ**: ゲートは出さず、インストール可能な場合のみ非ブロッキングのバナーを表示

standalone表示・セーフエリア(ノッチ/Dynamic Island/ホームバー)・オフライン起動に対応しています。

開発・検証用バイパス: `localhost` / Playwright(`navigator.webdriver`)では自動でゲートが無効。`?pwa-gate=on` で強制表示、`?pwa-gate=off` で強制バイパスできます(`src/lib/pwa.ts`)。
